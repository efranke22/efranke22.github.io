...
a <- records %>%
  ...
  geom_vline(xintercept = as.Date.character("06-01-2016", tryFormats = c("%m-%d-%Y")),
             linetype = "dashed")+
  annotate("text",x = as.Date("2009-01-01"),y = 140, color = "black",
           label = "First super shoe\nreleased",
           hjust = 0.5, vjust = 1, size = 3.5, fontface = "italic") +
  geom_point(alpha=0.6, color = "#3FA0FF")+
  geom_step(direction = "hv", color = "#3FA0FF", alpha = 0.3)+
  scale_y_continuous(labels = minutes_to_hm2)+
  scale_x_date(limits = as.Date(c("1950-01-01", "2025-01-01")), breaks = as.Date(c("1950-01-01", "1960-01-01", "1970-01-01", "1980-01-01", "1990-01-01", "2000-01-01", "2010-01-01", "2020-01-01")), labels = c("1950", "1960", "1970", "1980", "1990", "2000", "2010", "2020"))+
  theme_bw()+
  theme(axis.text = element_text(size = 10))+
  labs(x="Year", y="Marathon time (h:mm)")
...
b <- records %>%
  ...
  annotate("text",x = as.Date("1985-06-30"),y = 220, color = "gray47",
...
library(patchwork)
p <- a/b + plot_layout(axis_titles = "collect")
```
...
ggplot(combined %>% mutate(`Number of participants` = participants)) + 
  ...
  theme(axis.text = element_blank(),
    ...
    plot.margin = margin(0, 0, 0, 0, "pt"), # Set all margins to 0 points
...
chicago %>%
  ...
  geom_line() +
  geom_point()+
  annotate("text",x = 2020,y = 9900, color = "black",
           label = "First super shoe\nreleased",
           hjust = 0.5, vjust = 1, size = 3.5, fontface = "italic") +
  geom_ribbon(aes(ymin = avg_time - se_time, ymax = avg_time + se_time), alpha = 0.2, color = NA) +
  ...
  labs(y = "Average Finish Time (h:mm)",
...
ggplot() +
  ...
  annotate("text",x = 2020.5,y = 18000, color = "black",
           label = "First super shoe\nreleased",
           hjust = 0.5, vjust = 1, size = 3.5, fontface = "italic") +
  ...
  theme(legend.position = "bottom", 
        ...
        axis.text.y.right = element_text(color = "darkgray", size = 10), 
        axis.text = element_text(size = 10)) 
```
